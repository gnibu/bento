...
def install(repo, config, scope="project", purge=False, auto_pr=False):
    repo = repo.resolve()
    ...
    if scope == "personal":
        ...
        command_factory = lambda event, auto: guard + hook_command(event, auto)
    ...
    after = merge_json_hooks(before, purge, auto_pr, command_factory)
    if before == after:
        ...
        print(f"bento: {action} Bento SessionStart and Stop hooks in {path}")
    if scope == "project" and not purge:
        print("bento: commit .claude/settings.json so new worktrees inherit the hooks")
...
def main():
    ...
    try:
        ...
        install(repo, config, args.scope, args.purge, args.auto_pr)
