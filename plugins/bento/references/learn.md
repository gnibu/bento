...
`python3 "<plugin>/scripts/sessions.py"`: it prints each worktree session since the branch was
...
- **Preserve generated-file ownership.** If `.bento-state/baseline.json` lists the
  ...
  then run `python3 "<plugin>/scripts/l2-state.py" --repo . merge <proposal.json>`
  to preview; append `--apply` only after approval. `<plugin>` is the plugin root resolved
  ...
  `<plugin>/references/init.md` for the proposal format and recovery rules.
...
- **Manually** — `/bento:learn` (Claude) or `$bento:learn` (Codex), for
