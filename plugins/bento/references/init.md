...
`commands/init.md` is the generation workflow. Claude invokes it as
...
known bento plugin installation. The examples below run from the target repo root;
...
```sh
python3 -m unittest discover -s plugins/bento/scripts -p 'test_l2_state.py' -v
```
...
```sh
claude plugin eval ./plugins/bento --runs 3 --ablation with-without --no-publish
```
